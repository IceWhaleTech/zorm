...
Insert/Update operations now support both pointer and non-pointer struct/slice types for more flexible usage patterns:
...
Use concise struct tags to support auto-increment primary keys with backward compatibility:
...
JOIN operations support both string and condition object formats for powerful query capabilities:
...
Built-in transaction management with context support:
...
Automatically generate and create database tables from struct definitions:
...
Configurable pool settings for high-concurrency scenarios:
...
Master-slave architecture with automatic operation routing:
